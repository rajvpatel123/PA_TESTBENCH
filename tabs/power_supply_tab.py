# tabs/power_supply_tab.py - COMPLETE FILE
...
class PowerSupplyTab(ttk.Frame):
    ...
    def set_aliases(self, alias_map: dict):
        """Called from main after device tab loads aliases — repopulates table."""
    ...
    def _ch_label(self, supply_name: str, ch: int) -> str:
        """Returns 'PS4 CH1  (Keysight_E36234A)' or 'Keysight_E36234A CH1'."""
    ...
    # ── UI ─────────────────────────────────────────────────────
    def _build_ui(self):
        ttk.Label(self, text="Power Supply Configuration",
        ...
        cols = ("Channel", "Role", "Mode",
                "Set V", "Set A", "Prot Limit",
                "Meas V", "Meas A", "Output")
        self.tree = ttk.Treeview(left, columns=cols,
                                  show="headings", height=14)
        col_widths = {
            "Channel":    250, "Role":  70, "Mode":  50,
            "Set V":       80, "Set A": 80, "Prot Limit": 100,
        ...
        ttk.Label(left,
                  text="Double-click a row to configure  |  "
                       "Meas V / Meas A update during live readback",
                  foreground="gray").pack(pady=2)

        # ── Set / On / Off buttons ─────────────────────────────
        act_frame = ttk.LabelFrame(
            left, text="Channel Actions  (select a row first)")
        act_frame.pack(fill="x", padx=5, pady=4)

        row_a = ttk.Frame(act_frame)
        row_a.pack(fill="x", padx=10, pady=6)

        ttk.Button(row_a, text="Set Values",
                   command=self._set_selected).pack(side="left", padx=(0, 6))
        ttk.Label(row_a,
                  text="Sends V / I + protection limit to hardware. "
        ...
        tk.Button(row_b, text="  ON  ",
                  bg="#1a7a1a", fg="white",
                  activebackground="#145a14", activeforeground="white",
                  font=("Segoe UI", 10, "bold"),
                  command=self._output_on_selected).pack(side="left", padx=(0, 6))
        tk.Button(row_b, text="  OFF  ",
                  bg="#8b0000", fg="white",
                  activebackground="#5a0000", activeforeground="white",
                  font=("Segoe UI", 10, "bold"),
                  command=self._output_off_selected).pack(side="left", padx=(0, 12))
        ttk.Label(row_b,
        ...
        ttk.Label(rb_frame, text="Readback interval (ms):").pack(
            side="left", padx=(0, 5))
        self.rb_interval_var = tk.StringVar(value=str(READBACK_INTERVAL_MS))
        ttk.Entry(rb_frame, textvariable=self.rb_interval_var,
                  width=7).pack(side="left", padx=5)
        self.rb_btn = ttk.Button(rb_frame, text="Start Live Readback",
                                  command=self._toggle_readback)
        self.rb_btn.pack(side="left", padx=10)
        ttk.Button(rb_frame, text="Read Once",
                   command=self._read_once).pack(side="left", padx=5)

        # ── RIGHT: pairing panel ───────────────────────────────
        right = ttk.LabelFrame(main_frame, text="Gate / Drain Pairing")
        right.pack(side="right", fill="y", padx=(5, 0))

        ttk.Label(right, text="Available Gate channels:").pack(
            anchor="w", padx=5, pady=(10, 0))
        self.gate_list = tk.Listbox(right, height=6, width=38,
                                     selectmode="single",
                                     exportselection=False)
        self.gate_list.pack(padx=5, pady=2)

        ttk.Label(right, text="Available Drain channels:").pack(
            anchor="w", padx=5, pady=(8, 0))
        self.drain_list = tk.Listbox(right, height=6, width=38,
                                      selectmode="single",
                                      exportselection=False)
        self.drain_list.pack(padx=5, pady=2)

        ttk.Button(right, text="Pair Selected",
                   command=self._pair_selected).pack(fill="x", padx=5, pady=4)
        ttk.Button(right, text="Unpair Selected",
                   command=self._unpair_selected).pack(fill="x", padx=5, pady=2)

        ttk.Separator(right, orient="horizontal").pack(
            fill="x", padx=5, pady=8)

        ttk.Label(right, text="Active Pairs:",
    ...
    def _populate_channels(self):
        # Preserve existing config vars if repopulating for alias refresh
        ...
        for ps in POWER_SUPPLIES:
            ...
            for ch in range(1, ps["channels"] + 1):
                ...
                self.tree.insert("", "end", iid=ch_id,
                                 values=(ch_label,
                                         self._channels[ch_id]["role"],
                                         self._channels[ch_id]["mode"],
                                         "", "", "", "---", "---", "OFF"))
    ...
    def _open_edit_dialog(self, ch_id: str):
        ...
        ttk.Label(dialog, text=info["label"],
        ...
        role_frame.grid(row=1, column=0, columnspan=2,
                        padx=12, pady=4, sticky="ew")
        ttk.Label(role_frame, text="Role:", width=14,
                  anchor="e").grid(row=0, column=0, padx=8, pady=6, sticky="e")
        ...
        # ── Value rows (dynamic based on mode) ─────────────────
        vals_frame = ttk.LabelFrame(dialog, text="Values")
        vals_frame.grid(row=3, column=0, columnspan=2,
                        padx=12, pady=4, sticky="ew")

        # CV rows
        lbl_volt = ttk.Label(vals_frame, text="Set Voltage (V):",
                              width=20, anchor="e")
        ent_volt = ttk.Entry(vals_frame, textvariable=info["volt_var"],
                              width=14)
        lbl_ocp  = ttk.Label(vals_frame, text="OCP Limit (A):",
                              width=20, anchor="e")
        ent_ocp  = ttk.Entry(vals_frame, textvariable=info["ocp_var"],
                              width=14)

        # CC rows
        lbl_curr = ttk.Label(vals_frame, text="Set Current (A):",
                              width=20, anchor="e")
        ent_curr = ttk.Entry(vals_frame, textvariable=info["curr_var"],
                              width=14)
        lbl_ovp  = ttk.Label(vals_frame, text="OVP Limit (V):",
                              width=20, anchor="e")
        ent_ovp  = ttk.Entry(vals_frame, textvariable=info["ovp_var"],
                              width=14)
        ...
        def _refresh_val_rows():
            ...
            if mode_var.get() == "CV":
                ...
                gate_warn.grid(row=2, column=0, columnspan=2,
                               padx=8, pady=(0, 6))
            else:
        ...
        # ── Idq targeting ──────────────────────────────────────
        idq_frame = ttk.LabelFrame(
            dialog, text="Idq Targeting  (Drain channel only)")
        idq_frame.grid(row=4, column=0, columnspan=2,
                       padx=12, pady=4, sticky="ew")

        def idq_row(parent, label, var, row, hint=""):
            ttk.Label(parent, text=label, width=22,
        ...
        idq_row(idq_frame, "Target Idq (mA):",
                info["target_idq_ma"],    0, "Leave blank to skip")
        idq_row(idq_frame, "Tolerance ± (mA):",
                info["idq_tolerance_ma"], 1, "Default: 5 mA")
        idq_row(idq_frame, "Gate Step Size (mV):",
                info["idq_step_mv"],      2, "Default: 50 mV")
        idq_row(idq_frame, "Hard Abort > (mA):",
                info["max_idq_ma"],       3, "Leave blank = 3× target")
        ...
        ttk.Button(btn_frame, text="Set Values",
                   command=set_now).pack(side="left", padx=6)
        ttk.Button(btn_frame, text="Set + ON",
                   command=set_and_on).pack(side="left", padx=6)
    ...
    def _update_tree_row(self, ch_id: str):
        ...
        if mode == "CV":
            ...
            set_a   = ""
            prot    = (f"OCP {info['ocp_var'].get()} A"
                       if info["ocp_var"].get() else "")
        else:
            set_v   = ""
            set_a   = info["curr_var"].get()
            prot    = (f"OVP {info['ovp_var'].get()} V"
                       if info["ovp_var"].get() else "")
        self.tree.item(ch_id, values=(
            info["label"], info["role"], mode,
            set_v, set_a, prot,
    ...
    def _pair_selected(self):
        ...
        if not gate_sel or not drain_sel:
            messagebox.showwarning("Select Both",
                                   "Select one Gate and one Drain channel.")
            return
        ...
        self.pairs_list.insert(
            tk.END,
            f"GATE: {self._channels[gate_id]['label']}"
            f"  <->  DRAIN: {self._channels[drain_id]['label']}")
        self._refresh_pairing_lists()
        self.status_lbl.config(
            text=f"Paired {gate_label} <-> {drain_label}",
            foreground="green")
    ...
    def _unpair_selected(self):
        ...
        self._refresh_pairing_lists()
        self.status_lbl.config(
            text=f"Unpaired {gate_id} <-> {drain_id}", foreground="gray")
    ...
    def _set_channel_values(self, ch_id: str):
        """
        Send voltage/current + protection limit to hardware.
        Does NOT change output state — use _channel_output() for that.

        CV mode: set_voltage(V) + set_ocp(A)
        CC mode: set_current(A) + set_ovp(V)
        """
        info = self._channels[ch_id]
        drv  = self._registry.get(info["supply"])
        if drv is None:
            messagebox.showerror("Not Connected",
                                 f"{info['supply']} not in driver registry.")
            return
        try:
            if info["mode"] == "CV":
                ...
                self.status_lbl.config(
                    text=f"Set {info['label']}: {volts} V  "
                         f"| OCP = {ocp_str or 'unchanged'} A",
                    foreground="blue")
                _logger.info(
                    f"Set {ch_id} CV: {volts} V  OCP={ocp_str}")
            else:
                curr_str = info["curr_var"].get().strip()
                ovp_str  = info["ovp_var"].get().strip()
                if not curr_str:
                    messagebox.showwarning("Missing Value",
                                           f"Enter a current for {ch_id}.")
                    return
                amps = float(curr_str)
                ...
                self.status_lbl.config(
                    text=f"Set {info['label']}: {amps} A  "
                         f"| OVP = {ovp_str or 'unchanged'} V",
                    foreground="blue")
                _logger.info(
                    f"Set {ch_id} CC: {amps} A  OVP={ovp_str}")
    ...
    def _channel_output(self, ch_id: str, enable: bool):
        """Turn channel output ON or OFF independently of set values."""
        info = self._channels[ch_id]
        drv  = self._registry.get(info["supply"])
        if drv is None:
            messagebox.showerror("Not Connected",
        ...
        try:
            ...
            self.status_lbl.config(
                text=f"{info['label']} output {state}", foreground=color)
    ...
    def _toggle_readback(self):
        if self._polling:
            ...
            self.status_lbl.config(
                text="Status: Readback stopped", foreground="gray")
        else:
            try:
                interval_ms = int(self.rb_interval_var.get())
            except ValueError:
                messagebox.showerror("Invalid Interval",
                                     "Interval must be an integer (ms).")
                return
            self._polling = True
            self.rb_btn.config(text="Stop Live Readback")
            self.status_lbl.config(
                text="Status: Readback running...", foreground="orange")
            threading.Thread(target=self._readback_loop,
                             args=(interval_ms,), daemon=True).start()

    def _readback_loop(self, interval_ms: int):
        while self._polling:
            for ch_id in list(self._channels.keys()):
                if not self._polling:
                    break
                self.after(0, lambda c=ch_id: self._read_channel(c))
            time.sleep(interval_ms / 1000.0)

    def _read_once(self):
        for ch_id in self._channels:
            self._read_channel(ch_id)
        self.status_lbl.config(
            text="Status: Readback complete", foreground="green")

    def _read_channel(self, ch_id: str):
        info = self._channels[ch_id]
        drv  = self._registry.get(info["supply"])
        vals = list(self.tree.item(ch_id, "values"))
        if drv is None:
            ...
            return
        try:
            meas_v = drv.measure_voltage(info["channel"])
            vals[6] = f"{meas_v:.4f}"
        except Exception as e:
            vals[6] = "ERR"
            _logger.warning(f"Voltage readback failed {ch_id}: {e}")
        try:
            meas_a = drv.measure_current(info["channel"])
            vals[7] = f"{meas_a:.4f}"
        except Exception as e:
            vals[7] = "ERR"
    ...
    def get_pairs(self) -> list:
        ...
        for gate_id, drain_id in self._pairs:
            ...
            # Gate: CV → volt_var is the gate voltage target
            #       CC → curr_var is the set current (unusual but supported)
            if gi["mode"] == "CV":
            ...
            # Drain: CV → ocp is the current limit passed to sequencer
            #        CC → curr_var is the set current
            if di["mode"] == "CV":
