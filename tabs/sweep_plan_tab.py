...
ACTION_DEFAULTS = {
    ...
    "output_on":           {"channel": ""},
    "output_off":          {"channel": ""},
    "bias_sweep":          {"channel": "", "start": -6.0,
                            "stop": -2.0, "step": 0.5, "dwell_ms": 500},
    "idq_optimize":        {"drain_channel": "", "gate_channel": "",
                            "target_ma": 100.0, "tolerance_ma": 5.0,
                            "step_mv": 50.0, "max_ma": ""},
    "power_sweep":         {"start_dbm": -20.0, "stop_dbm": 10.0,
                            "step_db": 1.0, "dwell_ms": 200},
    "perform_measurement": {"notes": ""},
    "gain_compression":    {"compression_db": 1.0, "start_dbm": -20.0,
                            "stop_dbm": 20.0, "step_db": 0.5},
    "acpr":                {"center_hz": 1e9, "offset_hz": 5e6,
                            "bw_hz": 1e6},
    "harmonics":           {"fundamental_hz": 1e9, "num_harmonics": 3},
    "pae_sweep":           {"start_dbm": -20.0, "stop_dbm": 10.0,
                            "step_db": 1.0, "dwell_ms": 200},
    "freq_sweep":          {"start_hz": 1e9, "stop_hz": 4e9,
                            "step_hz": 500e6, "power_dbm": 0.0},
    ...
    "message":             {"text": ""},
    "cond_abort":          {"channel": "", "threshold_ma": 500.0,
                            "condition": ">"},
    "scpi_command":        {"instrument": "", "command": ""},
    "scpi_poll":           {"instrument": "", "query": "",
                            "expected": "", "timeout_s": 10.0},
}
...
class SweepPlanTab(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        ttk.Entry(toolbar, textvariable=self._title_var,
    ...
    def _set_run_status(self, state: str):
        """state: 'running', 'stopped', 'complete', 'error'"""
        styles = {
    ...
    def _build_sidebar_category(self, parent, category: str, actions: list):
        ...
        for display_name, action_type in actions:
            ttk.Button(
                action_container,
                text=f"  + {display_name}",
                command=lambda at=action_type, dn=display_name:
                    self._add_step(at, dn)
    ...
    def _build_plan_list(self, parent):
        ...
        self.plan_tree = ttk.Treeview(
            parent, columns=cols, show="headings", height=22)
    ...
    def _build_props_panel(self):
        ...
        vsb    = ttk.Scrollbar(self._props_frame, orient="vertical",
        ...
        self._props_canvas_id = canvas.create_window(
        ...
        canvas.bind("<Configure>",
                    lambda e: canvas.itemconfig(
                        self._props_canvas_id, width=e.width))
    ...
    def _show_props(self, idx: int):
        ...
        ttk.Separator(self._props_inner,
                      orient="horizontal").pack(fill="x", padx=8, pady=6)
    ...
    def _channel_row(self, parent, label: str, var: tk.StringVar, row: int):
        ...
        cb.grid(row=row, column=1, columnspan=2,
    ...
    def _apply_btn(self, parent, row: int, fn):
        ttk.Button(parent, text="Apply to Step",
                   command=fn).grid(
                   row=row, column=0, columnspan=3,
                   pady=12, padx=8, sticky="ew")
    ...
    def _props_set_gate_bias(self, idx: int):
        ...
        cv_rb = ttk.Radiobutton(f, text="CV — Constant Voltage", variable=modevar, value="CV")
        cc_rb = ttk.Radiobutton(f, text="CC — Constant Current", variable=modevar, value="CC")
        cv_rb.grid(row=1, column=0, columnspan=3, padx=16, sticky="w")
        cc_rb.grid(row=2, column=0, columnspan=3, padx=16, sticky="w")
            ...
            try:
                if modevar.get() == "CV":
                    ...
                    self._plan[idx]["params"].update(
                        mode="CV", channel=chvar.get(), voltage=v, ocp=a)
                else:
                    c = float(currvar.get())
                    o = float(ovpvar.get())
                    self._plan[idx]["params"].update(
                        mode="CC", channel=chvar.get(), current=c, ovp=o)
            except ValueError:
    ...
    def _props_ramp_updown(self, idx: int):
        ...
        override_frame.grid(row=2, column=0, columnspan=3,
            ...
            if not use_var.get():
                try:
                    ...
                    messagebox.showerror("Invalid Input",
                                         "Voltage and OCP must be numbers.")
                    return
            self._refresh_plan_tree()
            self._set_status(
                f"Step {idx+1}: "
                f"{'Ramp Editor' if use_var.get() else 'Manual override'}")
    ...
    def _props_output_onoff(self, idx: int):
        ...
        tk.Label(f, text="ON" if is_on else "OFF",
                 ...
                 width=6).grid(row=0, column=0, columnspan=3,
                               padx=8, pady=(4, 8))

        ttk.Label(f,
                  text="Turn ON the selected channel output"
                       if is_on else
                       "Turn OFF the selected channel output",
                  foreground="gray",
                  font=("Segoe UI", 8)).grid(
                  row=1, column=0, columnspan=3,
                  padx=8, pady=(0, 8))
        ...
        def apply():
            ...
            self._refresh_plan_tree()
            self._set_status(
                f"Step {idx+1}: Output {'ON' if is_on else 'OFF'} "
                f"→ {ch_var.get()}")
    ...
    def _props_bias_sweep(self, idx: int):
        ...
        preview_lbl = ttk.Label(f, text="", foreground="gray",
                                font=("Segoe UI", 8))
        preview_lbl.grid(row=6, column=0, columnspan=3, padx=8, pady=(0, 4))

        def _update_preview(*_):
            try:
                n = abs(float(stop_var.get()) -
                        float(start_var.get())) / float(step_var.get())
                preview_lbl.config(
                    text=f"≈ {int(n)+1} points  "
                         f"est. {(int(n)+1)*int(dwell_var.get())/1000:.1f} s")
        ...
        def apply():
            try:
                params = {
                    ...
                    "max_ma": float(max_var.get())
                               if max_var.get().strip() else "",
                }
            except ValueError:
                messagebox.showerror("Invalid Input",
            ...
            self._refresh_plan_tree()
            self._set_status(
                f"Step {idx+1} updated: Idq optimize {params['target_ma']} mA")
    ...
    def _props_gain_compression(self, idx: int):
        ...
        def apply():
            ...
            self._refresh_plan_tree()
            self._set_status(
                f"Step {idx+1} updated: P{params['compression_db']}dB compression")
    ...
    def _props_acpr(self, idx: int):
        ...
        self._field_row(f, "Channel Offset (MHz):", offset_var, 2,
                        hint="Distance to adj channel")
        self._field_row(f, "Channel BW (MHz):",     bw_var,     3,
                        hint="Integration bandwidth")
    ...
    def _props_harmonics(self, idx: int):
        ...
        self._field_row(f, "Num Harmonics:",     n_var,    2,
                        hint="2 = 2nd only,  3 = 2nd+3rd")
    ...
    def _props_freq_sweep(self, idx: int):
        ...
        def _update_preview(*_):
            try:
                n = abs(float(stop_var.get()) -
                        float(start_var.get())) * 1e3 / float(step_var.get())
    ...
    def _props_load_measurement(self, idx: int):
        ...
        ttk.Button(f, text="Browse…",
                   command=browse).grid(row=1, column=2, padx=4, pady=4)
    ...
    def _props_save_results(self, idx: int):
        ...
        self._field_row(f, "Filename:", name_var, 1,
                        hint="Blank = auto timestamp", width=22)
    ...
    # ══════════════════════════════════════════════════════════
    #  GENERIC FALLBACK (Flow + SCPI — replaced in Step 9+)
    # ══════════════════════════════════════════════════════════
    def _props_generic(self, idx: int):
        ...
        for key, val in step["params"].items():
            ...
            ttk.Label(row, text=f"{key}:", width=20,
    ...
    def _move_step(self, direction: int):
        ...
        if 0 <= new_idx < len(self._plan):
            self._plan[idx], self._plan[new_idx] = \
                self._plan[new_idx], self._plan[idx]
            self._refresh_plan_tree()
            self._select_plan_row(new_idx)
    ...
    def _new_plan(self):
        if self._plan:
            if not messagebox.askyesno("New Plan",
                                       "Discard current plan and start new?"):
                return
        self._plan         = []
        self._current_file = None
    ...
    def _update_file_label(self):
        if self._current_file:
            self._file_lbl.config(
                text=f"[{os.path.basename(self._current_file)}]",
                foreground="gray")
        else:
            self._file_lbl.config(text="[unsaved]", foreground="orange")

    # ══════════════════════════════════════════════════════════
    #  RUN / STOP — stubs until Step 12
    # ══════════════════════════════════════════════════════════
    def _run_plan(self):
        if not self._plan:
    ...
    def set_aliases(self, alias_map: dict):
        self._aliases = alias_map  # {device_name: alias_label}
    ...
    def get_sweep_plan_rows(self) -> list:
        ...
        for step in self._plan:
            ...
            if t == "set_gate_bias":
                ...
                rows.append({"command": "OUTPUT_ON", "params": {
                    "channel": p.get("channel", "")
                }})

            elif t == "output_off":
                rows.append({"command": "OUTPUT_OFF", "params": {
                    "channel": p.get("channel", "")
                }})

            elif t == "wait":
                rows.append({"command": "WAIT", "params": {
                ...
                # Pass through anything else (message, loop, etc.)
                rows.append({"command": t.upper(), "params": p})

        return rows

# ══════════════════════════════════════════════════════════════
#  STEP SUMMARY LINES  (module level — outside class)
# ══════════════════════════════════════════════════════════════
