...
class DeviceInfoTab(ttk.Frame):
    def __init__(self, parent):
        ...
        self._stats_vars = {
            ...
            "connected": tk.StringVar(value="0"),
    ...
    def _build_ui(self):
        ...
        # Top summary row
        summary = ttk.Frame(top_shell, style="Panel.TFrame")
        summary.pack(fill="x", pady=(0, 10))
        summary.configure(padding=10)

        total_card = make_stat_card(summary, "Configured devices", self._stats_vars["total"].get())
        total_card.pack(side="left", padx=(0, 10))

        connected_card = make_stat_card(summary, "Connected", self._stats_vars["connected"].get())
        connected_card.pack(side="left", padx=(0, 10))

        errors_card = make_stat_card(summary, "Attention needed", self._stats_vars["errors"].get())
        errors_card.pack(side="left")

        self._total_value_lbl = total_card.winfo_children()[0]
        self._connected_value_lbl = connected_card.winfo_children()[0]
        self._errors_value_lbl = errors_card.winfo_children()[0]
        ...
        right = ttk.LabelFrame(content, text="Selected Device", style="Card.TLabelframe")
        ...
        ttk.Button(toolbar, text="Connect All", command=self._connect_all, style="Primary.TButton").pack(side="left", padx=(0, 8))
        ttk.Button(toolbar, text="Reconnect Selected", command=self._reconnect_selected).pack(side="left", padx=4)
        ttk.Button(toolbar, text="Self-Test", command=self._self_test_selected).pack(side="left", padx=4)
        ttk.Button(toolbar, text="Reset", command=self._reset_selected).pack(side="left", padx=4)
        ttk.Button(toolbar, text="Edit Alias", command=self._edit_alias).pack(side="left", padx=4)
        ...
        self.tree = ttk.Treeview(table_panel, columns=cols, show="headings", height=16)
        ...
        for col in cols:
            ...
            anchor = "w" if col not in ("Status",) else "center"
        ...
        ttk.Label(right, textvariable=self.detail_name, font=("Segoe UI", 12, "bold")).pack(anchor="w", pady=(0, 8))
        self._detail_status_lbl = ttk.Label(right, textvariable=self.detail_status, foreground=APP_COLORS["muted"])
        ...
        ttk.Button(right, text="Reconnect Selected", command=self._reconnect_selected, style="Primary.TButton").pack(fill="x", pady=4)
        ttk.Button(right, text="Self-Test Selected", command=self._self_test_selected).pack(fill="x", pady=4)
        ttk.Button(right, text="Reset Selected", command=self._reset_selected).pack(fill="x", pady=4)
        ttk.Button(right, text="Edit Alias", command=self._edit_alias).pack(fill="x", pady=4)

        self.status_lbl = ttk.Label(self, text="Status: Ready", foreground=APP_COLORS["muted"])
        self.status_lbl.pack(anchor="w", padx=12, pady=(0, 12))

    def _detail_row(self, parent, label: str, var: tk.StringVar, wrap: bool = False):
        row = ttk.Frame(parent)
        row.pack(fill="x", pady=4)

        ttk.Label(row, text=f"{label}:", font=("Segoe UI", 9, "bold"), width=10).pack(side="left", anchor="n")
        ttk.Label(
            row,
            textvariable=var,
    ...
    def _connect_all(self):
        ...
        for inst in COMPANY_INSTRUMENTS:
            ok = self._connect_one(inst["name"], silent=True)
            if ok:
                connected += 1
            else:
                failed += 1
    ...
    def _connect_one(self, name: str, silent: bool = False):
        ...
        if not inst:
            if not silent:
                messagebox.showerror("Missing Device", f"No config found for {name}.")
            return False
        ...
        try:
            ...
            if not silent:
                messagebox.showerror("Connection Error", f"{inst['name']}:\n{e}")
            self._notify_driver_update()
            return False
    ...
    def _edit_alias(self):
        ...
        ttk.Label(dialog, text=name, font=("Segoe UI", 10, "bold")).grid(row=0, column=0, columnspan=2, padx=12, pady=(12, 4))
        ttk.Label(dialog, text="Alias:").grid(row=1, column=0, padx=10, sticky="e")

        alias_var = tk.StringVar(value=current_alias)
        ttk.Entry(dialog, textvariable=alias_var, width=28).grid(row=1, column=1, padx=10, pady=8)
        ...
        ttk.Button(dialog, text="Apply", command=apply, style="Primary.TButton").grid(row=3, column=0, pady=10, padx=10)
    ...
    def _reset_selected(self):
        ...
        try:
            ...
            set_status_label(self.status_lbl, f"Status: {name} reset", "success")
    ...
    def _set_row_status(self, item_id, vals, status: str, tone: str = ""):
        ...
        tags = []
        if tone == "ok":
            tags.append("status_ok")
        elif tone == "err":
            tags.append("status_err")
        elif tone == "warn":
    ...
    def _refresh_stats(self):
        ...
        for iid in self.tree.get_children():
            ...
            status = (vals[6] or "").lower()
            if "connected" in status:
                connected += 1
            elif any(x in status for x in ("error", "fail")):
                errors += 1

        self._stats_vars["total"].set(str(total))
        self._stats_vars["connected"].set(str(connected))
        self._stats_vars["errors"].set(str(errors))

        self._total_value_lbl.config(text=str(total))
        self._connected_value_lbl.config(text=str(connected))
        self._errors_value_lbl.config(text=str(errors))
